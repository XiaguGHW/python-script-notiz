...
## 2.1 当前有效性验证 / Source Validation

搜索到一个岗位后，必须实际验证其当前来源，不能仅凭搜索摘要、缓存或旧索引判断为开放岗位。

### 验证顺序

1. **实际打开岗位详情页或官方 PDF。**
   - 能被搜索引擎搜到不等于现在仍可访问。
   - 如果详情页或 PDF 返回 404 / 410 / removed / file not found，则不能标记为 current/open。

2. **同时检查 Institute 当前 Jobs / Karriere / HiWi 汇总页。**
   - 如果具体岗位页面仍存在且当前汇总页也继续列出，优先判断为 current。
   - 如果具体页面/PDF仍可访问，但当前汇总页已不再列出，至少标记为 `old_or_unclear`，除非页面本身有明确仍开放的证据。

3. **PDF 必须实际读取。**
   - 不能只依赖搜索摘要中的 PDF 文本。
   - 如果 PDF 可打开，则检查发布时间、截止日期、Beginn、联系人和是否仍有开放性表述。

4. **搜索引擎只用于发现，不用于最终确认。**
   - 搜索缓存、摘要、旧索引只能证明“曾经存在”。
   - 不能单独证明岗位“现在仍开放”。

5. **访问失败必须单独记录。**
   - 页面暂时访问失败 ≠ 岗位关闭。
   - 若仅是网络/服务器错误，标记 `access_failed / status_unclear`，不要直接判 CLOSED。
   - 若官方源明确返回永久性 404/410，且当前汇总页也不再列出，则可判 `closed_or_removed`。

### 允许进入“当前最新 HiWi 清单”的岗位

只有满足以下之一的岗位才进入当前推荐清单：

- 当前官方 Jobs/HiWi 页面明确列出；
- 当前详情页明确表示仍开放；
- 当前 PDF + 当前汇总页共同支持其仍有效；
- 对 Initiativanfrage，必须明确标记为 `initiative_target`，不能伪装成当前公开 vacancy。

旧公告、404 岗位、无法确认开放状态的历史页面只保留在 `known_jobs.json` 用于去重，不进入当前推荐榜。
...
- 搜索结果可发现岗位，但只有当前官方来源可验证后，才能进入“当前最新 HiWi 清单”
