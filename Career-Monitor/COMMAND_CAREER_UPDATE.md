...
- 优先接受可核验发布日期距离扫描日 **不超过 20 个自然日** 的岗位；
- 若无可核验发布日期，只要官方职位页面或官方招聘系统明确显示申请截止日期，且截止日仍在扫描日之后、并且不超过未来 **3 个月**，也可进入主推荐榜；
- 发布日期与截止日期均须来自官方职位页面、官方招聘系统或官方 PDF；搜索引擎抓取日期、社交媒体或第三方招聘网站日期仅用于发现，不能替代官方证据；
- 已超过官方截止日期的岗位记录为 `excluded_expired_deadline`；官方发布日期超过 20 天且没有符合条件的未来截止日期时记录为 `excluded_stale_posting`；
- 官方页面同时未显示可靠发布日期和未来 3 个月内的明确截止日期时，记录为 `status_unclear`，保留用于去重和待确认区，但不进入主推荐榜。
...
详细职位 MD 必须至少包含：职位标题的德语原文、与原文对应的中文翻译、德语职责/要求原文（可按条目摘录，但不得只写中文转述）、中文说明、来源与核验状态。
